...
dat.orig[1:5,]
mean(dat.orig$age)
mean(subset(dat.orig,sex==1,"age")) #Durchschnittsalter der Frauen
mean(subset(dat.orig,sex==0,"age")) #Durchschnittsalter der Männer
...
##ii
cwage <- cut(wrate, breaks=c(0,mean(wrate),max(wrate)),
  include.lowest=T,prob=T, labels=1:2)
prop.table(table(sex,cwage),2)[1,1]*100

##iii
cwage <- cut(wrate, breaks=quantile(seq(0,1,.1)),
  include.lowest=T,prob=T)
